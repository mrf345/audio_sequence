...
var checkFiles = function checkFiles (files = []) {
  ...
  for (var i = 0; files.length > i; i += 1) {
    ...
    }, 500) // time out is needed to get the right response after the files are fully loaded
  }
}
...
var audio_sequence = function AS (options) {
  ...
  this.options = {
    ...
    repeats: options.repeats || 0, // number of repeats to obay with some adjustments later
    repeat_whole: options.repeat_whole || 'true', // repeat all files as whole for the number of repeats
...
// Core functions
  ...
  this.__init__ = function __init__ () {
    ...
    if (!checkType('number', [this.options.repeats,
      this.options.repeat_delay])) throw new TypeError('audio_sequence(options) repeats and delay require number')
    ...
    if (this.options.auto_start === 'true') {
      if (this.options.repeat_whole === 'true' && this.options.repeat_each === 'false') this.whole_repeater()
      if (this.options.repeat_each === 'true') this.each_repeater()
    }
  }
  ...
  this.each_repeater = function eachRepeater (
    ...
    } else { this.abort(); if (this.options.cleanup === 'true') this.empty() } // make sure all created elements events are off and element removed
  }
...
// Monitoring and controlling audio
  ...
  this.replay = function replay () {
    ...
    this.__init__() // lets not reinvent the wheel !
  }
  ...
  this.unmute = function unmute () {
    ...
    $.each(this.defaults.elementsID, function (number, value) {
      if (elementExist(value)) {
        if (document.getElementById(value).volume === 0) document.getElementById(value).volume = '1'
      }
    })
...
// Managing files
  ...
  this.remove_file = function removeFile (id) {
    ...
    if (this.defaults.elementsID[id] !== 'undefined') {
      ...
      this.defaults.permID.splice(this.defaults.permID.indexOf(this.defaults.permID(id)), 1)
      return true
    } else return false
  }
